...
class PretrainedViTWrapper(nn.Module):
    ...
    def create_model(
        ...
        if is_dv3:
            ...
            model = torch.hub.load(
                "dinov3", "dinov3_vits16plus", source="local", weights=path
            )
            return model, None
    ...
    def forward_features(
        self, x: torch.Tensor, make_2D: bool = False, add_reg: bool = False
    ) -> torch.Tensor:
        b, _, h, w = x.shape
        p = self.patch_size
        s = self.stride
...
class AlibiVitWrapper(PretrainedViTWrapper):
    def __init__(
        ...
        **kwargs,
        ...
        #     # keep the attribute for compatibility but set to None or delete
        #     # delattr(self.model, "cls_token")
        #     self.model.cls_token = None

        # pos = self.model.pos_embed  # nn.Parameter
        # # create new pos_embed that excludes the cls position
        # new_pos = nn.Parameter(pos.data[:, 1:, :].clone())
        # self.model.pos_embed = new_pos
